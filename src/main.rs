...
fn make_controller_mouse_directory_and_config_file(controller_mouse_path: String) {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.text_edit_singleline(
                &mut "https://github.com/makingstan/controller_mouse/blob/master/README.md",
            );

            if ui.button("Update Data").clicked() {
                fs::write(
                    home::home_dir().unwrap().to_str().unwrap().to_owned()
                        + "/.controller_mouse/config.txt",
                    (102 - self.sensivity).to_string()
                        + "\n".as_str()
                        + self.vid.as_str()
                        + "\n".as_str()
                        + self.pid.as_str(),
                )
                .expect("Could not write to config.txt");
            }
        });
    }
}
...
fn main() {
    make_controller_mouse_directory_and_config_file(
